	...
	const FVector2D& MousePosition) const
{
	...
	EInv_TileQuadrant HoverTileQuadrant {EInv_TileQuadrant::None};
	if (bIsTop && bIsLeft) HoverTileQuadrant = EInv_TileQuadrant::TopLeft;
	else if (bIsTop && !bIsLeft) HoverTileQuadrant = EInv_TileQuadrant::TopRight;
	else if (!bIsTop && bIsLeft) HoverTileQuadrant = EInv_TileQuadrant::ButtonLeft;
	else if (!bIsTop && !bIsLeft) HoverTileQuadrant = EInv_TileQuadrant::ButtonRight;
	...
	const int32 StackAmount)
{
	...
	UInv_SlottedItem* SlottedItem = CreateSlottedItem(Item, bStackable, StackAmount, GridFragment, IconFragment, Index);
...
UInv_SlottedItem* UInv_InventoryGrid::CreateSlottedItem(UInv_InventoryItem* Item, const bool bStackable,
	const int32 StackAmount, const FInv_GridFragment* GridFragment, const FInv_IconFragment* IconFragment,
	const int32 Index)
{
	UInv_SlottedItem* SlottedItem = CreateWidget<UInv_SlottedItem>(GetOwningPlayer(), SlottedItemClass);
	SetSlottedItemImage(SlottedItem, GridFragment, IconFragment);
	SlottedItem->SetInventoryItem(Item);
	SlottedItem->SetGridIndex(Index);
	SlottedItem->SetIsStackable(bStackable);
	const int32 StackableAmount = bStackable ? StackAmount : 0;
	SlottedItem->UpdateStackCount(StackableAmount);
...
void UInv_InventoryGrid::SetSlottedItemImage(const UInv_SlottedItem* SlottedItem, const FInv_GridFragment* GridFragment,
                                             const FInv_IconFragment* IconFragment) const
{
	FSlateBrush Brush;
...
bool UInv_InventoryGrid::IsInGridBounds(const int32 StartIndex, const FIntPoint& ItemDimension) const
{
...
void UInv_InventoryGrid::OnPopUpMenuConsume(int32 Index)
{
	...
	const int32 UpperLeftGridIndex = GridSlots[Index]->GetUpperLeftIndex();
	UInv_GridSlot* UpperLeftGridSlot = GridSlots[UpperLeftGridIndex];
	const int32 NewStackCount = UpperLeftGridSlot->GetStackCount() - 1;
	
	UpperLeftGridSlot->SetStackCount(NewStackCount);
	SlottedItems.FindChecked(UpperLeftGridIndex)->UpdateStackCount(NewStackCount);
